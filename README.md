<p align="left">
  <img src="https://raw.githubusercontent.com/HrafnSognir/HrafnSognir/main/Assests/cbannr.svg" width="65%">
</p>


#### Exploring low-level programming through small C projects.

[![C](https://custom-icon-badges.demolab.com/badge/--%23282828.svg?logo=c&logoColor=%23d5c4a1)](https://www.c-language.org)
[![Raylib](https://custom-icon-badges.demolab.com/badge/Raylib-%23282828.svg?logo=raylib&logoColor=%23d5c4a1)](https://www.raylib.com)
[![Raygui](https://custom-icon-badges.demolab.com/badge/RayGUI-%23282828.svg?logo=raylib&logoColor=%23d5c4a1)](https://github.com/raysan5/raygui)
[![gtk](https://custom-icon-badges.demolab.com/badge/GTK-%23282828.svg?logo=gtk&logoColor=%23d5c4a1)](https://www.gtk.org)
[![ncurses](https://custom-icon-badges.demolab.com/badge/ncurses-%23282828.svg?logo=ncurses&logoColor=%23d5c4a1)](https://invisible-island.net/ncurses/)

---

### **_Current Focus:_**
• C fundamentals

### **_Planed Projects:_**
- TO-DO CLI tool
- Weahter forecast CLI tool

---

### **_C resources_:**
- [Effective C](https://www.amazon.com/Effective-Introduction-Professional-Robert-Seacord/dp/1718501048)
- [C Documentation](https://devdocs.io/c/)
- [Beej's Guide to C Programming](https://beej.us/guide/bgc/)<br>
  - [C Programming and OOP with Dr. Chuck](https://youtu.be/PaPN51Mm5qQ)
  - [C Programming Full Course](https://youtu.be/xND0t1pr3KY?list=PL5Sdu-vX2R3XPedgh8q0tBYuRTVWXOkMH)

---

### **_Frameworks resources:_**
- [Ncurses Documentation](https://tldp.org/HOWTO/NCURSES-Programming-HOWTO/)
- [GTK Documentation](https://docs.gtk.org/gtk4/getting_started.html)
- [GTK 4 Tutorial for beginners](https://github.com/ToshioCP/Gtk4-tutorial)
  - [Gtk4/GObject Tutorial Playlist](https://www.youtube.com/playlist?list=PLFnRB0VoVErpBiFZlMg_JAPKdJJobzyV3)
  - [Ncurses Playlist](https://www.youtube.com/playlist?list=PLdNUbYq5poiUdmMdOgRLHneZn4NMc4yEX)

---

### **_Game Development resources:_**
- [Coding with Sphere's Intro to Raylib](https://www.youtube.com/watch?v=AniAoJC6QAc&t)
- [Raylib Documentation](https://www.raylib.com/cheatsheet/cheatsheet.html)
- [Game Programming Patterns](https://gameprogrammingpatterns.com/)<br>
  - [C Programming with Raylib – Match3 Game Tutorial](https://youtu.be/P7PMA3X1tf8?list=PL5Sdu-vX2R3XPedgh8q0tBYuRTVWXOkMH)

---

<details>
  <summary><strong><span style="font-size: 1.1em;">How to Run any project:</span></strong></summary>
  <br>
  
  1. ##### Clone repo:
     ```
     git clone https://github.com/HrafnSognir/Taake_learns_to_C.git
     ```

  2. ##### Go into the project folder:
     ```
     cd path/to/project
     ```

  3. ##### Compile the program using GCC:
     ```
     gcc <file>.c -o <app>
     ```

  4. ##### Run the program:
     ```
     ./<app>
     ```
</details>

[![MIT License](https://img.shields.io/badge/License-MIT-%23d5c4a1)](https://github.com/HrafnSognir/Taake_learns_to_C?tab=MIT-1-ov-file)
